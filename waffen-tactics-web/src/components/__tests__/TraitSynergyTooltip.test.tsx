...
describe('TraitSynergyTooltip canonical lifecycle rendering', () => {
  ...
  it('renders every canonical tier and effect for every active trait', () => {
    const container = document.createElement('div')
    document.body.appendChild(container)

    for (const traitData of canonicalTraits) {
      const effectCount = traitData.modular_effects.flat().length
      act(() => {
        root = createRoot(container)
        root.render(
          <TraitSynergyTooltip
      ...
      const tooltip = document.body.querySelector('[data-trait-tooltip]') as HTMLElement
      expect(tooltip).not.toBeNull()
      expect(tooltip.querySelectorAll('[data-trait-effect-details]')).toHaveLength(effectCount)
      expect(tooltip.textContent).not.toContain('Do review')
      expect(tooltip.textContent).not.toMatch(/\d[.,]?\d*\/\d/)
