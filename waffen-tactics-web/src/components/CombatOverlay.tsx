...
const getTraitColor = (tier: number) => {
  ...
  // Dynamic coloring based on tier
  const tierColors = ['#6b7280', '#10b981', '#3b82f6', '#a855f7', '#f59e0b']
  
  if (tier >= tierColors.length - 1) {
    return tierColors[tierColors.length - 1] // Max color for highest tiers
  }
...
export default function CombatOverlay({ onClose }: Props) {
  ...
  const [combatSpeed, setCombatSpeed] = useState(() => {
    ...
    return saved ? parseFloat(saved) : 1 // Default to 1x
  })
  ...
  useEffect(() => {
    ...
    console.log('Starting combat stream to:', `${API_URL}/game/combat`)
    ...
    eventSource.onmessage = (event) => {
      console.log('📨 Combat event:', event.data)
      try {
        const data: CombatEvent = JSON.parse(event.data)
        
        // Collect all events in queue
        setEventQueue(prev => [...prev, data])
        
        // Start playing if not already playing
        if (data.type === 'units_init') {
          setIsPlaying(true)
        }
        ...
        console.error('Error parsing combat event:', error)
      }
    }
  ...
  // Process event queue with controlled timing
  useEffect(() => {
    ...
    const processNextEvent = () => {
      setEventQueue(prev => {
        ...
        if (nextEvent.type === 'units_init') {
          ...
          if (nextEvent.target_id && nextEvent.target_hp !== undefined) {
            if (nextEvent.target_id.startsWith('opp_')) {
              setOpponentUnits(prev => prev.map(u => 
                u.id === nextEvent.target_id ? { ...u, hp: nextEvent.target_hp! } : u
              ))
            } else {
          ...
          // Store gold breakdown but don't display yet (will show on Continue click)
        ...
        if (nextEvent.message) {
          setCombatLog(prev => [...prev, nextEvent.message!])
        }
    ...
    const getEventDelay = (event: CombatEvent) => {
      if (event.type === 'units_init') return 500 // Fixed 0.5s initial delay
    ...
    <div style={{ 
      ...
      <div style={{
        ...
        <div className="bg-gradient-to-r from-red-900 to-red-700 p-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1">
              ...
              {opponentInfo && (
                <div className="text-center text-sm text-red-100 mt-1">
                  vs {opponentInfo.name} (Lvl {opponentInfo.level}, {opponentInfo.wins} W)
                </div>
              )}
            </div>
            <div className="flex-1 flex justify-end items-center gap-2 pr-2">
              <div className="text-xs text-red-100 font-semibold">⚡ Tempo:</div>
              <input
                type="range"
                min="-10"
                max="10"
                ...
                onChange={(e) => {
                  // Logarithmic scale: slider -10 to 10 maps to 0.1x to 10x
                ...
                className="w-32 h-2 bg-red-800 rounded-lg appearance-none cursor-pointer"
                style={{
                  accentColor: '#fbbf24'
                }}
              />
        ...
        <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', flex: 1 }}>
                <div className="text-center">
                  ...
                  <div className="text-lg font-bold text-green-400">
                    {playerUnits.filter(u => u.hp > 0).length} / {playerUnits.length}
                  </div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-lg font-bold text-red-400">
                    {opponentUnits.filter(u => u.hp > 0).length} / {opponentUnits.length}
                  </div>
                </div>
              </div>
              {/* Active Traits */}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem' }}>
                    {Object.entries(synergies).map(([name, data]) => (
                      <div
                        key={name}
                        onMouseEnter={() => setHoveredTrait(name)}
                        ...
                        style={{
                          ...
                          fontSize: '0.75rem',
                          fontWeight: 'bold',
                          backgroundColor: `${getTraitColor(data.tier)}30`,
                          border: `1.5px solid ${getTraitColor(data.tier)}`,
                          color: getTraitColor(data.tier),
                          cursor: 'pointer',
                          transition: 'all 0.2s'
                        }}
                      >
                        ...
                        {hoveredTrait === name && (
                          <div style={{
                            ...
                            padding: '0.5rem',
                            backgroundColor: '#1e293b',
                            border: `2px solid ${getTraitColor(data.tier)}`,
              ...
              {playerUnits.map(unit => (
                <div
                  ...
                  style={{
                    ...
                    transition: 'all 0.3s',
                    boxShadow: attackingUnit === unit.id ? '0 0 20px #ff0000, 0 0 30px #ff0000' : 
                               targetUnit === unit.id ? '0 0 20px #ffff00, 0 0 30px #ffff00' :
                               unit.hp > 0 ? getRarityGlow(unit.cost) : 'none',
                ...
                >
                  ...
                  <div className="space-y-0.5 text-[9px] text-gray-300 mb-1">
                    <div className="flex justify-between">
                      <span>❤️ HP</span>
                      <span className="font-bold">{Math.floor(unit.hp)}/{Math.floor(unit.max_hp)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>⚔️ ATK</span>
                      <span className="font-bold">{unit.attack}</span>
                    </div>
                  </div>
            ...
            <div style={{ 
              ...
              {opponentUnits.map(unit => (
                ...
                >
                  ...
                  <div style={{ fontSize: '0.55rem', textAlign: 'center', color: '#fbbf24' }}>
                    {'⭐'.repeat(unit.star_level)}
                  </div>

                  {/* Stats Display */}
                  <div style={{ fontSize: '0.5rem', color: '#9ca3af', textAlign: 'center' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 0.125rem' }}>
                      <span>❤️</span>
                      <span style={{ fontWeight: 'bold' }}>{Math.floor(unit.hp)}/{Math.floor(unit.max_hp)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 0.125rem' }}>
                      <span>⚔️</span>
                      <span style={{ fontWeight: 'bold' }}>{unit.attack}</span>
                    </div>
                  </div>
                  ...
                  <div className="relative h-2 bg-gray-700 rounded-full overflow-hidden border border-gray-600">
                    <div
                      className="absolute inset-y-0 left-0 transition-all duration-300"
                      style={{
                        width: `${unit.max_hp > 0 ? (unit.hp / unit.max_hp) * 100 : 0}%`,
                        background: `linear-gradient(to right, ${getRarityColor(unit.cost)}, ${getRarityColor(unit.cost)}dd)`
                      }}
                    />
                  </div>
          ...
          {/* Combat Log */}
          <div className="bg-gray-800 rounded-lg p-2 border border-gray-700" style={{ 
            display: 'flex',
            flexDirection: 'column',
            height: '100px',
            flexShrink: 0
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              ...
              <button 
                ...
                style={{ fontSize: '0.65rem', padding: '0.125rem 0.5rem', backgroundColor: '#374151', borderRadius: '0.25rem', color: '#9ca3af', border: 'none', cursor: 'pointer' }}
              >
                {showLog ? 'Ukryj' : 'Pokaż'}
              </button>
            </div>
            {showLog && <div style={{ 
              flex: 1,
              overflowY: 'auto'
            }} className="space-y-1 text-xs text-gray-300 font-mono">
        ...
        <div className="bg-gray-800 p-4 border-t border-gray-700">
          {isFinished ? (
            <button
              onClick={() => {
                // Show gold notification first
                if (storedGoldBreakdown) {
                  setDisplayedGoldBreakdown(storedGoldBreakdown)
                }
                handleClose()
              }}
              className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white font-bold py-3 px-6 rounded-lg transition-all"
            >
              Kontynuuj
            </button>
          ) : (
            <div className="text-center text-gray-400">
              <div className="inline-block animate-pulse">⚔️ Walka trwa...</div>
            </div>
          )}
        </div>
      </div>
      ...
      <GoldNotification 
        breakdown={displayedGoldBreakdown} 
        onDismiss={() => setDisplayedGoldBreakdown(null)} 
      />
    </div>
  )
}
