...
class CombatEffectProcessor:
    ...
    @staticmethod
    def _normalise_stat_buff_stats(effect: Dict[str, Any]) -> List[str]:
        """Return the canonical stat list used by every stat-buff path."""
        stats = effect.get('stats')
        if not stats:
            singular_stat = effect.get('stat')
            stats = [singular_stat] if singular_stat else []

        if not isinstance(stats, list) or not stats:
            raise ValueError('stat_buff requires a non-empty stats list or stat field')
        if any(not isinstance(stat, str) or not stat for stat in stats):
            raise ValueError('stat_buff stats must contain non-empty strings')
        return stats

    @staticmethod
    def _max_buff_amplifier(unit: 'CombatUnit') -> float:
        """Read the strongest authored buff amplifier without mutating state."""
        multipliers = [
    ...
    def _calculate_stat_buff_increment(
        self,
        unit: 'CombatUnit',
        stat_type: str,
        value: float,
        ...
        """Calculate one absolute increment before recipient application.

        Percentage values are anchored to the source unit, matching the
        historical combat contract. Percentage-valued custom stats keep their
        authored decimal representation (for example 25% lifesteal = 0.25).
        The handler receives the resulting absolute increment as a flat value,
        so percentage math is performed exactly once.
        """
        if not is_percentage:
            return value

        if stat_type in ('hp', 'hp_regen_per_sec'):
            base_value = getattr(unit, 'max_hp', 0)
        elif stat_type in ('lifesteal', 'damage_reduction'):
            return value / 100.0
        else:
            base_value = getattr(unit, stat_type, 0)

        return self.effect_processor.stat_calculator.calculate_buff_increment(
            0, value, 'percentage', base_value
        )

    @staticmethod
    def _recipient_hp_context(
        source_unit: 'CombatUnit',
        recipient: 'CombatUnit',
        hp_list: List[int],
        unit_idx: int,
        attacking_team: Optional[List['CombatUnit']],
        defending_team: Optional[List['CombatUnit']],
        attacking_hp: Optional[List[int]],
        defending_hp: Optional[List[int]],
    ) -> tuple[List[int], int]:
        """Resolve the HP mirror belonging to a recipient, not the trigger side."""
        if attacking_team is not None and recipient in attacking_team:
            idx = attacking_team.index(recipient)
            return (attacking_hp if attacking_hp is not None else (hp_list if recipient is source_unit else []), idx)
        if defending_team is not None and recipient in defending_team:
            idx = defending_team.index(recipient)
            return (defending_hp if defending_hp is not None else (hp_list if recipient is source_unit else []), idx)
        if recipient is source_unit:
            return hp_list, unit_idx
        return [], -1
    ...
    def _apply_stat_buff_with_handlers(
        ...
        """Apply all stat buffs through the shared resolver and handler registry."""
        stats = self._normalise_stat_buff_stats(effect)
        ...
        # Validate the full action before resolving or mutating any recipient.
        # This makes mixed supported/unsupported stat lists atomic at the
        # contract boundary and prevents a typo from partially applying.
        handlers = self.effect_processor.buff_handlers
        unsupported = [stat for stat in stats if stat not in handlers]
        if unsupported:
            raise ValueError(f"Unsupported stat-buff stat type(s): {', '.join(unsupported)}")
        ...
        source_amplifier = self._max_buff_amplifier(unit)
        increments = {
            stat_type: self._calculate_stat_buff_increment(
                unit, stat_type, val, buff_type == 'percentage'
            ) * source_amplifier
            for stat_type in stats
        }

        for stat_type in stats:
            handler = handlers[stat_type]

            for recipient in recipients:
                recipient_mult = self._max_buff_amplifier(recipient)
                final_increment = increments[stat_type] * recipient_mult
                recipient_hp_list, recipient_idx = self._recipient_hp_context(
                    unit,
                    recipient,
                    hp_list,
                    unit_idx,
                ...
                handler.apply_buff(
                    recipient,
                    final_increment,
                    False,  # percentage math was completed once above
                    1.0,  # both source and recipient amplifiers are applied above
                    recipient_hp_list,
                    recipient_idx,
                    time,
                    log,
    ...
    def _apply_stat_buff(
        ...
        """Apply stat buff from an effect through one canonical path."""
