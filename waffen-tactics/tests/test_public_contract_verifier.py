...
def test_verify_reports_malformed_public_traits(monkeypatch, tmp_path: Path):
    repo_root = tmp_path
    data_dir = repo_root / "waffen-tactics"
    data_dir.mkdir()
    (data_dir / "units.json").write_text(json.dumps({"units": [{"id": "u1"}]}), encoding="utf-8")
    (data_dir / "traits.json").write_text(
        json.dumps({"traits": [{"id": "t1", "name": "Trait", "type": "trait"}]}),
        encoding="utf-8",
    )
    (data_dir / "item_recipe_matrix_wft139.json").write_text(
        json.dumps({
    ...
    payloads = {
        "/api/game/units": [{"id": "u1"}],
        "/api/game/traits": [{"name": "Trait", "type": "trait"}],
    ...
    report = verifier.verify("https://example.test", repo_root)
    trait_result = next(contract for contract in report["contracts"] if contract["name"] == "traits")

    assert report["ok"] is False
    assert trait_result["actual_count"] == 1
    assert trait_result["missing_ids"] == ["t1"]
    assert trait_result["schema_errors"] == [
        "/api/game/traits[0] is missing required keys: ['id']",
        "/api/game/traits[0] has no non-empty string id",
    ]
