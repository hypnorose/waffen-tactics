...
class ReplayUnit:
    ...
    def apply_stat_buff(self, payload: dict):
        ...
        try:
            ...
            pass
        # update buffed_stats best-effort
        if stat:
            try:
...
def test_replay_full_stats_matches_snapshot():
    # Reuse Bosman-like teams for a representative simulation
    ...
    def make(uid, name, hp=600, attack=50):
        ...
        if u is None:
            ...
            stats = StatsObj(hp, mana_on_attack=0)
            return CU(id=uid, name=name, hp=hp, attack=attack, defense=0, attack_speed=1.0, stats=stats)
        u.id = uid
        return u
    ...
    assert snapshots, "No snapshots emitted"
    init = snapshots[0]
    final = snapshots[-1]

    # Build replay units from initial snapshot (both sides)
    replay = {}
    for u in (init.get('player_units', []) + init.get('opponent_units', [])):
        ru = ReplayUnit(u)
        replay[ru.id] = ru

    # Determine last seq to replay up to
    last_seq = final.get('seq') if isinstance(final.get('seq'), int) else None

    # Apply events in order (events already have seq attached by simulator)
    for et, payload in events:
        # skip snapshots themselves
        if et == 'state_snapshot':
            continue
        # stop if we've passed final snapshot seq
    ...
    # Now compare every canonical field for every unit in final snapshot
    def compare_unit_snapshot(u_snap):
        uid = u_snap.get('id')
        assert uid in replay, f"Unit {uid} missing in replay map"
        ru = replay[uid]
        # numeric comparisons (tolerant for floats)
        assert int(u_snap.get('hp', 0)) == int(ru.hp), f"HP mismatch for {uid}: snap={u_snap.get('hp')} replay={ru.hp}"
        assert int(u_snap.get('max_hp', 0)) == int(ru.max_hp), f"Max HP mismatch for {uid}"
        assert int(u_snap.get('attack', 0)) == int(ru.attack), f"Attack mismatch for {uid}"
        assert int(u_snap.get('defense', 0)) == int(ru.defense), f"Defense mismatch for {uid}"
        assert float(u_snap.get('attack_speed', 1.0)) == float(ru.attack_speed), f"Attack speed mismatch for {uid}"
        assert int(u_snap.get('current_mana', 0)) == int(ru.current_mana), f"Mana mismatch for {uid}"
        assert int(u_snap.get('max_mana', 0)) == int(ru.max_mana), f"Max mana mismatch for {uid}"
        assert int(u_snap.get('shield', 0)) == int(ru.shield), f"Shield mismatch for {uid}"
        # buffed_stats mapping
        snap_bs = u_snap.get('buffed_stats', {}) or {}
        for k, v in snap_bs.items():
            assert k in ru.buffed_stats, f"Missing buffed stat {k} for {uid}"
            try:
                assert float(v) == float(ru.buffed_stats.get(k)), f"Buffed stat {k} mismatch for {uid}: snap={v} replay={ru.buffed_stats.get(k)}"
            except Exception:
                pass
        # effects ids subset
        snap_ids = {e.get('id') for e in (u_snap.get('effects') or []) if e.get('id')}
        replay_ids = {e.get('id') for e in (ru.effects or []) if e.get('id')}
        assert snap_ids.issubset(replay_ids) or not snap_ids, f"Effect ids mismatch for {uid}"

    for pu in final.get('player_units', []) + final.get('opponent_units', []):
        compare_unit_snapshot(pu)
