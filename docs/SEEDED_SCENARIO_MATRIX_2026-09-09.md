...
The machine-readable coverage summary is generated by
`tools/seeded_coverage_report.py` and checked in at
`docs/SEEDED_SCENARIO_COVERAGE_REPORT_2026-09-10.json`.
...
- The coverage report records unit, trait, trigger-family, edge-case, and
  cross-layer evidence separately; it explicitly keeps authenticated Game
  View, deployment, and rollback evidence in `manual_runtime.pending`.
